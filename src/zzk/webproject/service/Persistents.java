...
public class Persistents {
    ...
    public static int doUpdate(String sql, Map<Integer, Object> parametersMap) {
        Connection connection = get();
        int update = -1;
        try {
            PreparedStatement preparedStatement = connection.prepareStatement(sql);
            if (parametersMap.size() > 0) {
                Set<Integer> keySet = parametersMap.keySet();
                for (Integer index : keySet) {
                    preparedStatement.setObject(index, parametersMap.get(index));
                }
            }
    ...
    public static <POJO> POJO doQuery(String sql, Map<Integer, Object> parametersMap, Function<ResultSet, POJO> resultSetFunction) {
        Connection connection = get();
        ...
        try {
            ...
            if (parametersMap.size() > 0) {
                Set<Integer> keySet = parametersMap.keySet();
                for (Integer index : keySet) {
                    prepareStatement.setObject(index, parametersMap.get(index));
                }
            }
